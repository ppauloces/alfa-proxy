        ...
        <template x-for="(card, idx) in cards" :key="idx">
            <div class="finance-card admin-card"
                ...
                <p class="text-2xl font-bold mt-2"
                    ...
                    x-text="card.value"></p>
                <p class="text-sm mt-1" :class="cardTrendClass(card)">
    ...
    <div class="admin-card mb-8" x-data="renovacaoVps()">
        ...
        <form @submit.prevent="submit()" class="grid md:grid-cols-5 gap-4 items-end">
            ...
            <label class="flex flex-col gap-1.5">
                ...
                <div class="relative">
                    <span
                        class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm font-semibold">R$</span>
                        ...
                        <div class="flex items-center justify-between py-3 px-3 rounded-xl hover:bg-slate-50 transition-colors"
                            ...
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 rounded-full flex items-center justify-center shrink-0"
                                    :class="saida.tipo === 'renovacao' ? 'bg-blue-100 text-blue-600' : (saida.tipo === 'uso_interno' ? 'bg-indigo-100 text-indigo-600' : (saida.tipo === 'substituicao' ? 'bg-orange-100 text-orange-600' : (saida.tipo === 'compra' ? 'bg-emerald-100 text-emerald-600' : 'bg-red-100 text-red-500')))">
                                    <i class="fas text-xs"
                                        :class="saida.tipo === 'renovacao' ? 'fa-sync-alt' : (saida.tipo === 'uso_interno' ? 'fa-briefcase' : (saida.tipo === 'substituicao' ? 'fa-exchange-alt' : (saida.tipo === 'compra' ? 'fa-server' : 'fa-arrow-down')))"></i>
                                </div>
                                <div>
                                    <div class="flex items-center gap-2">
                                        ...
                                        <template x-if="saida.tipo === 'renovacao'">
                                            <span
                                                class="inline-flex items-center gap-1 px-2 py-0.5 bg-gradient-to-r from-blue-100 to-cyan-100 border border-blue-200 rounded-full">
                                                <i class="fas fa-sync-alt text-[8px] text-blue-600"></i>
                                                <span
                                                    class="text-[9px] font-black text-blue-700 uppercase tracking-wider">Renovação</span>
                                            </span>
                                        </template>
                                        <template x-if="saida.tipo === 'compra'">
                                            <span
                                                class="inline-flex items-center gap-1 px-2 py-0.5 bg-gradient-to-r from-emerald-100 to-green-100 border border-emerald-200 rounded-full">
                                                <i class="fas fa-server text-[8px] text-emerald-600"></i>
                                                <span
                                                    class="text-[9px] font-black text-emerald-700 uppercase tracking-wider">Contratação</span>
                                            </span>
                                        </template>
                                        <template x-if="saida.tipo === 'uso_interno'">
                                            <span
                                                class="inline-flex items-center gap-1 px-2 py-0.5 bg-gradient-to-r from-indigo-100 to-purple-100 border border-indigo-200 rounded-full">
                                                <i class="fas fa-briefcase text-[8px] text-indigo-600"></i>
                                                <span
                                                    class="text-[9px] font-black text-indigo-700 uppercase tracking-wider">Uso
                                                    Interno</span>
                                            </span>
                                        </template>
                                        <template x-if="saida.tipo === 'substituicao'">
                                            <span
                                                class="inline-flex items-center gap-1 px-2 py-0.5 bg-gradient-to-r from-orange-100 to-red-100 border border-orange-200 rounded-full">
                                                <i class="fas fa-exchange-alt text-[8px] text-orange-600"></i>
                                                <span
                                                    class="text-[9px] font-black text-orange-700 uppercase tracking-wider">Subst.
                                                    de Proxy</span>
                                            </span>
                                        </template>
                                        <template x-if="saida.status === 'pendente'">
                                            <span
                                                class="inline-flex items-center gap-1 px-2 py-0.5 bg-amber-50 border border-amber-200 rounded-full">
                                                <span
                                                    class="text-[9px] font-black text-amber-700 uppercase tracking-wider">Pendente</span>
                                            </span>
                                        </template>
                                    </div>
                            ...
                            <span class="font-bold whitespace-nowrap"
                                :class="saida.tipo === 'uso_interno' ? 'text-indigo-500 font-mono text-xs' : (saida.tipo === 'substituicao' ? 'text-orange-500 font-mono text-xs' : 'text-red-500')"
                ...
                <div class="space-y-1 text-sm max-h-[480px] overflow-y-auto pr-1">
                    <template x-for="(entrada, idx) in entradas" :key="idx">
                        <div class="flex items-center justify-between py-3 px-3 rounded-xl hover:bg-slate-50 transition-colors"
                            ...
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-8 h-8 rounded-full flex items-center justify-center shrink-0 bg-emerald-100 text-emerald-600">
                                    <i class="fas fa-arrow-up text-xs"></i>
                                </div>
                                <div>
                                    <div class="flex items-center gap-2">
                                        <p class="font-semibold text-slate-900"
                                            x-text="entrada.quantidade + ' ' + (entrada.quantidade === 1 ? 'proxy' : 'proxies')">
                                        </p>
                                        <template x-if="entrada.is_revendedor">
                                            <span
                                                class="inline-flex items-center gap-1 px-2 py-0.5 bg-gradient-to-r from-amber-100 to-orange-100 border border-amber-200 rounded-full">
                                                <i class="fas fa-crown text-[8px] text-amber-600"></i>
                                                <span
                                                    class="text-[9px] font-black text-amber-700 uppercase tracking-wider">Revendedor</span>
                                            </span>
                                        </template>
                                    </div>
                                    <p class="text-xs text-slate-400 flex items-center gap-1">
                                        ...
                                        <span
                                            x-text="entrada.username + ' • ' + entrada.categoria + ' • ' + entrada.data"></span>
                                    </p>
                                </div>
                            </div>
                            <span class="text-emerald-500 font-bold whitespace-nowrap"
                                x-text="entrada.valor_total"></span>
                        </div>
                    </template>
                </div>
            </template>
