    ...
    <div class="admin-card">
        <div class="flex flex-wrap items-center justify-between gap-4 mb-4">
            ...
            <div class="flex gap-2">
                <button type="button" onclick="window.location.reload()" class="btn-secondary text-xs px-3 py-2"><i class="fas fa-sync-alt"></i> Atualizar</button>
                ...
                <tbody>
                    <template x-for="(proxy, idx) in filteredProxies" :key="proxy.id + '-' + idx">
                        <tr>
                            ...
                            <td>
                                <span class="font-mono text-xs font-bold text-slate-700 bg-slate-50 px-2 py-1 rounded-lg" x-text="proxy.endereco"></span>
                            </td>
                            <td>
                            ...
                            <td>
                                <template x-if="proxy.status === 'bloqueada'">
                                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-red-50 text-red-600 text-[10px] font-black uppercase tracking-wider border border-red-100">
                                        <i class="fas fa-ban text-[8px]"></i> Bloqueada
                                    </span>
                                </template>
                                <template x-if="proxy.status === 'ativa' && parseInt(proxy.periodo) <= 0">
                                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-50 text-slate-500 text-[10px] font-black uppercase tracking-wider border border-slate-200">
                                        <i class="fas fa-hourglass-end text-[8px]"></i> Expirada
                                    </span>
                                </template>
                                <template x-if="proxy.status === 'ativa' && parseInt(proxy.periodo) > 0">
                                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-emerald-50 text-emerald-600 text-[10px] font-black uppercase tracking-wider border border-emerald-100">
                            ...
                            <td>
                                ...
                                <template x-if="parseInt(proxy.periodo) > 0 && parseInt(proxy.periodo) <= 3">
                                    <span class="text-sm font-bold text-amber-600" x-text="parseInt(proxy.periodo) + 'd restantes'"></span>
                                </template>
                                <template x-if="parseInt(proxy.periodo) > 3">
                                    <span class="text-sm font-bold text-slate-700" x-text="parseInt(proxy.periodo) + 'd restantes'"></span>
                                </template>
                            </td>
                            <td>
                                <template x-if="proxy.valor_unitario">
                                    <span class="text-sm font-bold text-slate-900" x-text="'R$ ' + parseFloat(proxy.valor_unitario).toLocaleString('pt-BR', {minimumFractionDigits: 2})"></span>
                                </template>
                                <template x-if="!proxy.valor_unitario">
        ...
        <div class="relative bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto"
            ...
            <div class="px-6 pt-6 pb-4 border-b border-slate-100">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-[10px] font-black text-[#448ccb] uppercase tracking-[0.3em]">Detalhes da venda</p>
                ...
                <div class="mt-3 flex items-center gap-2">
                    <template x-if="sel?.status === 'ativa' && parseInt(sel?.periodo) > 0">
                        <div class="inline-flex items-center gap-2 px-3 py-1.5 bg-emerald-50 border border-emerald-200 rounded-xl">
                    ...
                    <template x-if="sel?.status === 'bloqueada'">
                        <div class="inline-flex items-center gap-2 px-3 py-1.5 bg-red-50 border border-red-200 rounded-xl">
            ...
            <div class="px-6 py-4">
                ...
                <div class="space-y-0">
                    ...
                    <div class="flex justify-between items-center py-2.5 border-b border-slate-50">
                        ...
                        <div class="flex items-center gap-2">
                            <span class="text-sm font-mono font-bold text-slate-900" x-text="showPass ? sel?.senha : '••••••••'"></span>
                            <button type="button" @click="showPass = !showPass" class="text-slate-400 hover:text-slate-600 transition-colors">
                                <i class="fas text-xs" :class="showPass ? 'fa-eye-slash' : 'fa-eye'"></i>
                            </button>
                        </div>
                    </div>
                    <div class="flex justify-between items-center py-2.5 border-b border-slate-50">
                        <span class="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Período restante</span>
                        <span class="text-sm font-bold" :class="parseInt(sel?.periodo) <= 0 ? 'text-red-500' : parseInt(sel?.periodo) <= 3 ? 'text-amber-600' : 'text-slate-900'" x-text="parseInt(sel?.periodo) > 0 ? parseInt(sel?.periodo) + ' dias' : 'Expirado'"></span>
                    </div>
            ...
            <div class="px-6 py-4 border-t border-slate-100">
                ...
                <div class="space-y-0">
                    ...
                    <div class="flex justify-between items-center py-2.5">
                        <span class="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Proxies compradas</span>
            ...
            {{-- Dados da Compra --}}
            <div class="px-6 py-4 border-t border-slate-100" x-show="sel?.valor_unitario || sel?.periodo_comprado || sel?.motivo">
                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">
                    <i class="fas fa-cart-shopping mr-1"></i> Dados da Compra
                </p>
                <div class="space-y-0">
                    <div class="flex justify-between items-center py-2.5 border-b border-slate-50" x-show="sel?.valor_unitario">
                        <span class="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Valor unitário</span>
                        <span class="text-sm font-bold text-slate-900" x-text="'R$ ' + parseFloat(sel?.valor_unitario || 0).toLocaleString('pt-BR', {minimumFractionDigits: 2})"></span>
                    </div>
                    <div class="flex justify-between items-center py-2.5 border-b border-slate-50" x-show="sel?.periodo_comprado">
                        <span class="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Período contratado</span>
                    ...
                    <div class="flex justify-between items-start py-2.5" x-show="sel?.motivo">
                        ...
                        <span class="text-sm font-bold text-slate-700 max-w-[200px] text-right" x-text="sel?.motivo"></span>
                    </div>
                </div>
            </div>
            ...
            <div class="px-6 py-4 border-t border-slate-100 flex items-center justify-between gap-3">
                <div class="flex items-center gap-2">
                    <button type="button"
                        ...
                        data-action="test-proxy"
                        :data-ip="sel?.ip"
                        :data-porta="sel?.porta"
                        :data-usuario="sel?.usuario"
                        :data-senha="sel?.senha">
    ...
    return {
        ...
        get filteredProxies() {
            ...
            return this.proxies.filter(p => {
                ...
                if (this.filterStatus === 'expirada') return parseInt(p.periodo) <= 0;
                return true;
            });
        },
